...
# ---------------------------------------------------------------------------
Lx, Ly = 12, 12              # 4x4 square lattice -> 16 sites
U       = 8.0              # on-site Hubbard repulsion (in units of the hopping t)
mu      = 3.0697448030836-0.5*U  # chemical potential
muT     = 0.7080485232936  # trial chemical potential (will be overwritten if tune_mu=True)
nt      = 400              # number of imaginary-time slices
dt      = 0.005             # imaginary-time step  ->  beta = nt*dt
...
target_dens = 0.75      # otherwise target_dens defaults to 0.0
...
def main():
    ...
    with working_dir(run_dir):
        ...
        with open("ftafqmc.json", "w") as jf:
            jf.write(build_json(wfn_file, ham_file, dt, nt, wtype, sweeps, nwlks_per_mpi, nPop, nOrtho, seed))
